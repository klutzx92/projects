    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                for (Place possible : sq.successors()) {
                    if (sq.connectable(model.get(possible))) {
                        nFound += 1;
                        found = model.get(possible);
                    }
                }
                if (nFound > 0) {
                    for (Place possible : sq.successors()) {
                        if (sq.sequenceNum() != 0
                                && model.solnNumToPlace(sq.sequenceNum() + 1)
                                        .equals(possible)) {
                            nFound = 1;
                            found = model.get(possible);
                        }
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                for (Place possible : sq.predecessors()) {
                    if (model.get(possible).connectable(sq)) {
                        nFound += 1;
                        found = model.get(possible);
                    }
                }
