...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int k = 1; k < last + 1; k += 1) {
            if (!isCell(_solnNumToPlace[k])) {
                throw new IllegalArgumentException(
                        "Not all numbers from 1 to last contained.");
            }
        }
        ...
        for (int y = 0; y < _height; y += 1) {
            for (int x = 0; x < _width; x += 1) {
                _board[x][y]._successors = _allSuccessors[x][y][get(x, y)._dir];
                _board[x][y]._predecessors = _allSuccessors[x][y][0];
            }
        }
        _unconnected = last - 1;
    }
    ...
    Model(Model model) {
        ...
        for (Sq sq : _allSquares) {
            if (sq._sequenceNum != 0) {
                if (sq._sequenceNum == 1) {
                    sq._predecessor = null;
                    sq._successor = get(_solnNumToPlace[sq._sequenceNum + 1]);
                } else if (get(sq)._sequenceNum == _height * _width) {
                    sq._predecessor = get(_solnNumToPlace[sq._sequenceNum - 1]);
                    sq._successor = null;
                } else {
                    sq._predecessor = get(_solnNumToPlace[sq._sequenceNum - 1]);
                    sq._successor = get(_solnNumToPlace[sq._sequenceNum + 1]);
        ...
        if (!solved()) {
            for (int y = 0; y < _height; y += 1) {
                for (int x = 0; x < _width; x += 1) {
                    for (Sq sq : _allSquares) {
                        if (sq != get(x, y)) {
                            if (get(x, y)._sequenceNum != 0 && sq._sequenceNum
                                    == get(x, y)._sequenceNum + 1) {
                                if (get(x, y).connectable(sq)) {
                                    get(x, y).connect(sq);
                                }
                            }
                        }
    ...
    private int arrowDirection(int x, int y) {
        int solnNum = _solution[x][y];
        if (solnNum == _height * _width) {
            return 0;
        } else {
            return Place.dirOf(x, y,
                    _solnNumToPlace[solnNum + 1].x,
                    _solnNumToPlace[solnNum + 1].y);
        }
    }
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            Sq s0 = this;
            if (pl(s0.x, s0.y) == pl(s1.x, s1.y)) {
                return false;
            }
            if (Place.dirOf(s0.x, s0.y, s1.x, s1.y) != s0.direction()) {
                return false;
            }
            if (s0.successor() != null || s1.predecessor() != null) {
                return false;
            }
            if (s0.sequenceNum() != 0 && s1.sequenceNum() != 0) {
                return s0.sequenceNum() == s1.sequenceNum() - 1;
            } else if (s0.sequenceNum() == 0 && s1.sequenceNum() == 0) {
                if (s0._group >= 0 || s1._group >= 0) {
                    return s0._group != s1._group;
                }
            }
            return true;
        }
        ...
        boolean connect(Sq s1) {
            ...
            Sq sqCopy = this._head;
            while (sqCopy != null) {
                sqCopy._head = this._head;
                sqCopy = sqCopy._successor;
            }
            if (this.sequenceNum() != 0) {
                releaseGroup(s1._group);
                sqCopy = this;
                while (sqCopy._successor != null) {
                    sqCopy._successor._sequenceNum = sqCopy.sequenceNum() + 1;
                    sqCopy._successor._group = sqCopy._group;
                    sqCopy = sqCopy._successor;
                ...
                sqCopy = s1;
                while (sqCopy != this._head) {
                    sqCopy._predecessor._sequenceNum = sqCopy.sequenceNum() - 1;
                    sqCopy._predecessor._group = sqCopy._group;
                    sqCopy = sqCopy._predecessor;
                }
            } else {
                int newG = joinGroups(this._group, s1._group);
                this._head._group = newG;
                sqCopy = this._head;
                while (sqCopy != null) {
                    sqCopy._group = this._head._group;
                    sqCopy = sqCopy._successor;
                }
            }
            return true;
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (this._predecessor == null && next._successor == null) {
                    ...
                    int newG = newGroup();
                    for (Sq sqCopy = next; sqCopy != null;
                         sqCopy = sqCopy.successor()) {
                        sqCopy._group = newG;
                    }
                }
            } else {
                if (!this.fixedSeqNumChecker("prev")) {
                    if (this._predecessor != null) {
                        int newG = newGroup();
                        for (Sq sqCopy = this; sqCopy != null;
                             sqCopy = sqCopy._predecessor) {
                            sqCopy._sequenceNum = 0;
                            sqCopy._group = newG;
                        }
                    } else {
                        this._sequenceNum = 0;
                ...
                if (!next.fixedSeqNumChecker("next")) {
            ...
            for (Sq sqCopy = next; sqCopy != null; sqCopy = sqCopy._successor) {
                sqCopy._head = next;
            }
        }

        /**
         * Helper method checks if there exists a fixed sequence number.
         * Method takes in a string to know which direction of instance
         * to check and returns whether it found a fixed sequence number.
         * Used in disconnect().
         *
         * @param str String that tells function which direction to look
         * @return hasFixedSequenceNum Boolean indicating if fixed found.
         */
        boolean fixedSeqNumChecker(String str) {
            boolean hasFixedSequenceNum = false;
            if (str.equals("prev")) {
                for (Sq sqCopy = this; sqCopy != null;
                     sqCopy = sqCopy._predecessor) {
                    if (sqCopy._hasFixedNum) {
                        hasFixedSequenceNum = true;
                    }
                }
            }
            if (str.equals("next")) {
